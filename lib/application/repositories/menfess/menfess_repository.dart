...
class MenfessRepository extends BaseMenfessRepository {
  ...
  @override
  Future<void> updateComment({required int id, required String body}) async {
    var res = await NetworkUtil.put(Uri.parse('${Constants.baseUrl}/comments/$id'), body: jsonEncode({
      'body': body,
    }));
  ...
  Future<void> deleteComment({required int id}) async {
    var res = await NetworkUtil.delete(Uri.parse('${Constants.baseUrl}/comments/$id'));
